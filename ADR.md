...
## ADR-0007: フロントエンドJSは.ts化せず、JSDoc型注釈 + checkJs で型検査する
- Status: Accepted
- Context: 2026-07-25、フロントエンドのJS（`event-config.js`, `analytics.js`, `app.js`, `admin.js`, `create/create.js`）にも型安全性が欲しくなった。しかし[[ADR-0006]]後もフロントエンドは「ビルドなしで`<script>`から直接配信する」構成を維持しており、`.ts`化するとトランスパイルのビルドステップが必須になってこの方針と衝突する。
- Decision: フロントエンドのファイルは`.js`のまま、JSDoc型注釈を付けて `tsconfig.frontend.json`（`allowJs` + `checkJs` + `strict`）で `tsc --noEmit` による検査だけを行う。バックエンドとはlibが衝突する（DOM vs `@cloudflare/workers-types`）ためtsconfigを分離し、`npm run typecheck` が両方を順に実行する。APIレスポンス等の共有シェイプは `types/frontend.d.ts` にグローバル型として定義し、バックエンド（`src/app.ts`, `src/lib/`）の型と手動で同期する。あわせて全フロントJSをIIFEでラップした（`app.js`/`admin.js`のトップレベル宣言が非モジュールスクリプトとしてグローバルスコープで衝突するため。実行時のグローバル汚染も解消される）。
- Consequences: 無ビルド配信（[[ADR-0006]]のフロント方針）を維持したまま型検査が得られ、CIやローカルで `npm run typecheck` 一発でフロント・バックエンド両方を検査できる。一方、型注釈はJSDocでやや冗長になり、`types/frontend.d.ts` とバックエンド型定義のズレはツールでは検出されないため手動同期が必要。
