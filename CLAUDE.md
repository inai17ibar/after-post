...
自動テストは存在しませんが、`npm run typecheck`（`tsc --noEmit`）でバックエンド(TypeScript)の型チェックはできます。`python -m http.server` だけではAPIが無いため動きません。コードを変更したら、必ず `npm run dev`（`wrangler pages dev .`）でローカルに立ち上げ、実際にブラウザで該当画面を触って確認してください（`AGENTS.md`の起動セクション参照）。「エラーが出ない」ことと「意図通り動く」ことは別物です。
